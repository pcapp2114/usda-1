...
/**
 * Tests to see whether .gitignore.txt files are correctly managed.
 *
 ...
 * repository's .gitignore.txt file.
 *
 * @group Scaffold
 */
class ManageGitIgnoreTest extends TestCase {
  ...
  protected function createSutWithGit($fixture_name) {
    ...
    // .gitignore.txt files will not be managed unless there is a git repository.
  ...
  /**
   * Tests scaffold command correctly manages the .gitignore.txt file.
   */
  public function testManageGitIgnore() {
    // Note that the drupal-composer-drupal-project fixture does not
    // have any configuration settings related to .gitignore.txt management.
    ...
    $this->assertFileDoesNotExist($sut . '/docroot/sites/.gitignore.txt');
...
/web.config
EOT;
    // At this point we should have a .gitignore.txt file, because although we did
    // not explicitly ask for .gitignore.txt tracking, the vendor directory is not
    // tracked, so the default in that instance is to manage .gitignore.txt files.
    $this->assertScaffoldedFile($sut . '/docroot/.gitignore.txt', FALSE, $expected);
    $this->assertScaffoldedFile($sut . '/docroot/sites/.gitignore.txt', FALSE, 'example.settings.local.php');
    $this->assertScaffoldedFile($sut . '/docroot/sites/default/.gitignore.txt', FALSE, 'default.services.yml');
    $expected = <<<EOT
M docroot/.gitignore.txt
?? docroot/sites/.gitignore.txt
?? docroot/sites/default/.gitignore.txt
EOT;
    // Check to see whether there are any untracked files. We expect that
    // only the .gitignore.txt files themselves should be untracked.
    $stdout = $this->mustExec('git status --porcelain', $sut);
    $this->assertEquals(trim($expected), trim($stdout));
  }

  /**
   * Tests scaffold command does not manage the .gitignore.txt file when disabled.
   */
  public function testUnmanagedGitIgnoreWhenDisabled() {
    // Note that the drupal-drupal fixture has a configuration setting
    // `"gitignore.txt": false,` which disables .gitignore.txt file handling.
    ...
    $this->assertFileDoesNotExist($sut . '/.gitignore.txt');
    $this->assertFileDoesNotExist($sut . '/docroot/sites/default/.gitignore.txt');
  }
   ...
   * .gitignore.txt files, then we expect that the unmanaged file should not be
   * added to the .gitignore.txt file, because unmanaged files should be committed.
   */
  public function testAppendToEmptySettingsIsUnmanaged() {
    ...
    $this->assertFileDoesNotExist($sut . '/sites/.gitignore.txt');
    // Run the scaffold command.
    $this->fixtures->runScaffold($sut);
    $this->assertFileExists($sut . '/autoload.php');
    $this->assertFileExists($sut . '/index.php');

    $this->assertScaffoldedFile($sut . '/sites/.gitignore.txt', FALSE, 'example.sites.php');
    $this->assertScaffoldedFileDoesNotContain($sut . '/sites/.gitignore.txt', 'settings.php');
  }

  /**
   * Tests scaffold command disables .gitignore.txt management when git not present.
   *
  ...
  public function testUnmanagedGitIgnoreWhenGitNotAvailable() {
    ...
    // configuration settings related to .gitignore.txt management.
    $sut = $this->createSutWithGit('drupal-composer-drupal-project');
    $this->assertFileDoesNotExist($sut . '/docroot/sites/default/.gitignore.txt');
    $this->assertFileDoesNotExist($sut . '/docroot/index.php');
    $this->assertFileDoesNotExist($sut . '/docroot/sites/.gitignore.txt');
