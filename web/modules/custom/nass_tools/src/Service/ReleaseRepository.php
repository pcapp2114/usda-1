...
final class ReleaseRepository {
  ...
  public function all(array $options = []): array {
    ...
    // Safety fallback: existing sites may already have older nass_release
    // configuration where the TX source is not present or is disabled. The
    // homepage and calendar must still be able to render the ingested TX data
    // shipped with / configured for nass_tools. If the configured source pass
    // returns no records, try the TX source directly before returning empty.
    if (!$records) {
      foreach ($this->txSource->fetch($options) as $record) {
        $array = is_object($record) && method_exists($record, 'toArray') ? $record->toArray() : $record;
        if (empty($array['release_datetime']) || empty($array['title'])) {
          continue;
        }
        $records[$this->dedupeKey($array)] = $array;
      }
    }
