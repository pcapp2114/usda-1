(function ($, Drupal, drupalSettings) {
  ...
  Drupal.behaviors.nassRelease = {
    attach: function (context, settings) {
      ...
      var upcomingwrap = $('#upcoming-release-data');

      if (upcomingwrap.length && upcomingData.length) {
        upcomingwrap.find('.upcoming-release-item, .upcoming-release-date').remove();
        var currentUpcomingDate = '';

        for (var u = 0; u < Math.min(upcomingData.length, 3); u++) {
          var upcoming = upcomingData[u];
          var upcomingTitle = upcoming.title || '';
          var upcomingUrl = upcoming.url || '';
          var upcomingDate = upcoming.formatted_date || '';
          var upcomingTime = upcoming.time || '';
          var upcomingTitleHtml = upcomingUrl ? '<a href="' + upcomingUrl + '">' + upcomingTitle + '</a>' : upcomingTitle;

          if (upcomingDate && upcomingDate !== currentUpcomingDate) {
            upcomingwrap.find('.upcoming-release-note').before(
              '<h3 class="release-main-date upcoming-release-date">' + upcomingDate.toUpperCase() + '</h3>'
            );
            currentUpcomingDate = upcomingDate;
          }

          upcomingwrap.find('.upcoming-release-note').before(
            '<div class="release-item-info upcoming-release-item" data-title="' + upcomingTitle + '">' +
              '<div class="release-time-title">' +
                '<div class="release-date-time">' + upcomingTime + '</div>' +
                '<h4>' + upcomingTitleHtml + '</h4>' +
              '</div>' +
            '</div>'
          );
        }
