...
final class ReleaseRenderBuilder {
  ...
  public function buildTodayBlock(): array {
    ...
    try {
      $today_releases = $this->repository->currentReportDay();
      $display_date = $today_releases ? substr($today_releases[0]['release_datetime'], 0, 10) : (new \DateTimeImmutable('now', $tz))->format('Y-m-d');
    ...
    $no_release_message = is_array($textarea) && !empty($textarea['value'])
      ? $textarea['value']
      : 'There are no NASS reports scheduled. Previous reports are available from our <a href="/data-and-statistics/calendar/release-calendar">Calendar</a> section.';
    ...
    return [
      ...
      '#attached' => [
        'library' => ['nass_release/nass_release'],
      ],
      '#cache' => [
        'max-age' => 0,
