  ...
  */
  ...
  public static function getVariableArray($class_name) {
    ...
    elseif ($class_name == "mo_options_enum_signin_settings") {
      $class_object = [
        'Base_URL_value'    => 'miniorange_oauth_client_base_url_checkbox',
      ];
    }
    return $class_object;
  }
...
/**
  ...
  /**
   * Checks base_url of site for Https callback.
   *
   * @return string
   *   Returns base url of site.
   */
  public static function getOAuthBaseURL($base_url){
    $baseUrlValueCheckbox = \Drupal::config('oauth_login_oauth2.settings')->get('miniorange_oauth_client_base_url_checkbox');
    if($baseUrlValueCheckbox){
      $baseUrlValue = preg_replace('/http:\/\//', 'https://', $base_url);
    }else{
    ...
    {
        $site_key = \Drupal::service('private_key')->get();
        $key = hash('sha256', $site_key, true);
    ...
    /**
     * This function is to decrypt the data
     *
     * @param $text
     * @return false|string
     */
    public static function decrypt($text): false|string
    {
        $site_key = \Drupal::service('private_key')->get();
        $key = hash('sha256', $site_key, true);
     ...
     *
     * @param string|null $secret encrypted client secret
     *
     * @return string
     */
    public static final function maskClientSecret(?string $secret): string
    {
        ...
        $secret = self::getClientSecret($secret);
        return substr($secret, 0, 3) . str_repeat('*', 20) . substr($secret, -3);
    }

    /**
     * The below function is to decrypt the
     *
     * @param $secret
     * @return string
     */
    public static final function getClientSecret($secret): string
    {
