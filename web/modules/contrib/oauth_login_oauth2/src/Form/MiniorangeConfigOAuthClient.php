...
class MiniorangeConfigOAuthClient extends FormBase {
  ...
  */
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    if ($status == 'callback') {
      if (!empty($config->get('miniorange_auth_client_callback_uri')) && (strpos($config->get('miniorange_auth_client_callback_uri'), 'default') === FALSE)) {
        ...
        $callbackUrl = $baseUrlValue . "/mo_callback";
      ...
      $form['markup_callback_markup'] = [
        '#markup' => '<h2>'.t('Step 1/3 - Setting up a Relying Party / OAuth Provider').'</h2><hr><br>',
      ];

      $form['markup_top_callback']['miniorange_oauth_client_name_attr_title'] = [
        '#markup' => t('Copy below-mentioned Callback/Redirect URL and configure it in your OAuth Provider.').'<br><br>
                                   <div class="container-inline"><div class="mo_oauth_attr_mapping_label mo-callback"><b>'.t('Callback/Redirect URL: ').'</b> </div>',
      ];
    ...
    elseif ($status == 'select_application') {
      ...
      $form['markup_top_vt_start1'] = [
        '#markup' => '<h3>'.t('CONFIGURE APPLICATION ').'<a class="button mo_top_guide_button" target="_blank" href="https://www.drupal.org/docs/contributed-modules/drupal-oauth-oidc-login">'.t('&#128366; Setup Guides').'</a><a class="button" target="_blank" href="https://youtube.com/playlist?list=PL2vweZ-PcNpcL0xy5LGD7X8LO2dblwHHl">'.t('&#x23E9; Video Guides').'</a></h3><hr>',
      ];
      ...
      $form['mo_markup_2'] = [
        '#markup' => '</div><div id="oauth_apps"><h4> '.t('OAuth 2.0 supported applications ').'</h4><hr></div>',
      ];
      ...
      $form['mo_markup_3'] = [
        '#markup' => '<div id="oidc_apps"><h4>'.t('OpenID Connect supported applications ').'<a href="licensing" class="mo-note-oidc">'.t('[Note: We support OpenID protocol in Premium and Enterprise version of the module] ').'</a></h4><hr></div>',
      ];

      $form['mo_application_list_oidc'] = [
        '#prefix' => '<ul id="mo_search_ul_oidc" class="mo-wrap mo-flex-container"><li class="mo-flex-item">',
        '#markup' => implode('</li><li class="mo-flex-item disabled">', $oidc_apps),
        '#suffix' => '</li></ul> ',
      ];

      $form['mo_markup_4'] = [
        '#markup' => '<h4>'.t('Custom OAuth/OIDC applications').'</h4><hr> <div id="provider_not_present" class="mo-select-app-margin mo_oauth_highlight_background_note_export"><strong>'.t('Note: If your provider is not listed, you can select custom provider to configure the module. Please send us a query using miniOrange icon').'<i> '.t('(bottom right corner)').'</i> '.t('if you need any help in the configuration.').'</strong></div> ',
      ];
    ...
    elseif ($status == 'client_credentials') {
      ...
      $form['mo_client_credentials'] = [
        '#markup' => '<h2>'.t('Step 2/3 - Client Credentials Setup').'</h2><hr><br>',
      ];
      ...
      $form['markup_top_client_credentials']['miniorange_oauth_client_display_name'] = [
        ...
        '#required' => TRUE,
        '#description' => '<b>'.t('Note:').'</b> '.t('The login link will appear on the user login page in this manner.'),
      ...
      $form['markup_top_client_credentials']['miniorange_oauth_client_id'] = [
        ...
        '#required' => TRUE,
        '#maxlength' => 1048,
        '#description' => t('You will get this value from your OAuth Server'),
        '#attributes' => $attributes_arr,
      ];
      $client_secret = $config->get('miniorange_auth_client_client_secret');
      if(!empty($client_secret)){
        $client_secret = Utilities::maskClientSecret($client_secret);
      }
    ...
    elseif ($status == 'endpoints') {
      ...
      $form['mo_endpoints'] = [
        '#markup' => '<h2>'.t('Step 3/3 - Scope and Endpoints').'</h2><hr><br>',
      ];
    ...
    elseif ($status == 'final' && $action == 'update') {
      ...
      $form['miniorange_oauth_client_msgs'] = [
        '#markup' => "<div class='mo_oauth_highlight_background_note_2'>
                                         <b class='mo_note_css'>".t('Please Note:').'</b> '.t('Attribute Mapping is mandatory for login. Select the Email Attribute from Test Configuration and click on the ').'<b>'.t('Done').'</b> '.t('button.').'</div><br>',
      ];

      $form['miniorange_oauth_client'] = [
        '#markup' => '<a data-dialog-type="modal" href="add_new_provider" class="use-ajax button button--primary add_new_provider">'.t('+ Add New Provider').'</a>',
      ];
      ...
      $form['mo_oauth_client_idplist_table'] = [
        '#type' => 'table',
        '#header' => [t('Provider Name'), t('Client ID'), t('Test'), t('Action'), t('Mapping')],
        '#empty' => '<b>'.t('You have not configured any provider yet, Please Add provider by clicking above \"Add New SP\" button').'</b>',
        '#prefix' => '<br>',
        '#suffix' => '</div>',
  ...
  public function fetchOauthEndpoints(array &$form, FormStateInterface $form_state) {
    ...
    \Drupal::messenger()->addStatus(Markup::create(t('Endpoints fetched successfully. Please click on the <b>All Done</b> button.')));
  }
  ...
  public function miniorangeOauthChangeApp(array &$form, FormStateInterface $form_state) {
    ConfirmDeleteForm::resetMoConfig($form, $form_state);
  }
  ...
  */
  ...
  public function miniOauthLoginFinalTable($client_app, $client_id, $base_url) {
    ...
    $row['mapping'] = [
      '#markup' => Markup::create('<a class="button button--small" href="config_clc?action=update&app=' . $client_app . '#attribute_mapping">'.$this->t('Configure').'</a>'),
    ];
  ...
  */
  ...
  public function miniorangeOauthClientTableData($key, $value, $endpoints) {
    ...
    if ($key == 'Grant Types') {
      ...
      $icon = '<a href="licensing"><img src="' . $url_path . '/pro.png" alt="ENTERPRISE" class="mo_shortened_image"><span class="mo_pro_tooltip">Available in the Standard, Premium and Enterprise version</span></a>';
    }
    $row[$key . $key] = [
      '#markup' => '<div class="container-inline mo-table_app1"><strong>' . $key . '</strong>'.($icon ?? ''),
    ];
    if ($key == 'Callback URL: ') {
      $row[$value] = [
        '#type' => 'item',
    ...
    elseif ($key == 'Enable OAuth SSO login: ') {
      $row[$value] = [
        '#type' => 'checkbox',
        '#default_value' => $config->get($value),
        '#title' => '<i>'.t('Note: Check this option to show SSO link on the Login page').'</i>',
      ];
    }
    elseif ($key == 'Client Credentials in Header: ') {
      $row[$value] = [
        '#type' => 'checkbox',
        '#default_value' => $config->get($value),
        '#title' => '<i>'.t(' Note: Check this option if you want to send Client ID and Secret in Headers').' </i>',
      ];
    }
    elseif ($key == 'Client Credentials in Body: ') {
      $row[$value] = [
        '#type' => 'checkbox',
        '#default_value' => $config->get($value),
        '#title' => '<i>'.t(' Note: Check this option if you want to send Client ID and Secret in Body').' </i>',
      ];
    }
    else if ($key == 'Grant Types') {
      $row[$value] = [
        '#type' => 'radios',
        '#options' => [
          'authorization_code' => self::grantTypeWithDescription(
            MiniorangeOAuthClientConstants::AUTH_CODE_GRANT, MiniorangeOAuthClientConstants::AUTH_CODE_GRANT_GUIDE
          ),
          'authorization_code_with_pkce' => self::grantTypeWithDescription(
            MiniorangeOAuthClientConstants::AUTH_CODE_PKCE_GRANT, MiniorangeOAuthClientConstants::AUTH_CODE_PKCE_GRANT_GUIDE
          ),
          'password' => self::grantTypeWithDescription(
            MiniorangeOAuthClientConstants::PASS_GRANT, MiniorangeOAuthClientConstants::PASS_GRANT_GUIDE
          ),
          'implicit' => self::grantTypeWithDescription(
            MiniorangeOAuthClientConstants::IMPLICIT_GRANT,
            MiniorangeOAuthClientConstants::IMPLICIT_GRANT_GUIDE
          ),
        ],
    ...
    elseif ($key == 'Client credentials in Body: ') {
      ...
      $row[$value] = [
        '#type' => 'textfield',
        '#required' => TRUE,
        '#default_value' => $value == 'miniorange_auth_client_client_secret' ? Utilities::maskClientSecret($config->get($value)) : $config->get($value),
        '#maxlength' => 1048,
        '#suffix' => '</div>',
      ];
    }
  ...
  */
  ...
  public function miniorangeOauthEndpoints(array &$form, FormStateInterface $form_state) {
    ...
    if(!empty($client_secret)){
      $client_secret = preg_match('/\*\*\*\*\*\*\*\*\*\*/', $client_secret) ?
          $configFactory->get('miniorange_auth_client_client_secret') :
          Utilities::encrypt($client_secret)
      ;
    }
  ...
  */
  ...
  public function miniorangeOauthFinalSummary(array &$form, FormStateInterface $form_state) {
    ...
    $configFactory->set('miniorange_oauth_enable_login_with_oauth', true)->save();
    $configFactory->set('miniorange_oauth_send_with_header_oauth', true)->save();
    \Drupal::messenger()->addStatus(Markup::create(t('Configurations saved successfully. Please click on the <b>Perform Test Configuration</b> button to test the connection.')));
  ...
  */
  ...
  public function miniorangeOauthCompleteConfiguration(array &$form, FormStateInterface $form_state) {
    ...
    $data = self::moDataConfigurations();
    unset($data['Grant Types']);
    foreach ($data as $key => $value) {
      $field_data = $form_values[$key][$value];
      if ($value == 'miniorange_auth_client_client_secret') {
        if(!empty($field_data)){
            $field_data = preg_match('/\*\*\*\*\*\*\*\*\*\*/', $field_data) ?
                $configFactory->get('miniorange_auth_client_client_secret') :
                Utilities::encrypt($field_data);
        }
      }
      $configFactory->set($value, $field_data)->save();
    }
    \Drupal::messenger()->addStatus(Markup::create(t('Configurations saved successfully. Please click on the <b>Perform Test Configuration</b> button to test the connection.')));
    $form_state->setRedirect('oauth_login_oauth2.config_clc');
  }
  ...
  */
  ...
  public static function callDiscoveryEndpoint($discovery_url) {
    ...
    $supported_scopes = isset($content['scopes_supported']) && !empty($content['scopes_supported']) ? $content['scopes_supported'] : [];

    $common_scopes = MiniorangeOAuthClientConstants::COMMON_SCOPES;

    $matched_scopes = array_intersect($supported_scopes, $common_scopes);

    if (!empty($matched_scopes)) {
        $final_scopes = $matched_scopes;
    } else {
        $final_scopes = $supported_scopes;
    }
    $endpoints['scopes'] = implode(' ', $final_scopes);
  ...
  */
  ...
  public function moDataConfigurations() {
    ...
    $data = [
      ...
      'Grant Types' => 'miniorange_oauth_grant_type',
  ...
  */
   ...
   *
   */
  public static function grantTypeWithDescription($grant_type, $guide, $suffix = ''){
    return Markup::create('<a href="'.$guide.'" target="_blank">'.$grant_type.'</a>'.$suffix);
  }
