...
class ElasticsearchTest extends BackendTest {
  ...
  public function testFramework() {
    if ($this->elasticsearchAvailable) {
      ...
      throw new \Exception('Error: The Elasticsearch instance could not be found.', 1);
    }
  }
  ...
  protected function searchSuccess1() {
    ...
    $this->assertEquals($results->getResultItems()[$id]->getDatasourceId(), 'entity:entity_test');
    ...
    $this->assertEquals(array_keys($results->getResultItems()), $this->getItemIds(array(4)), 'Complex search 1 returned correct result.');
    $this->assertIgnored($results);
    $this->assertWarnings($results);
  }
  ...
  protected function checkModuleUninstall() {
    ...
    $this->assertEquals($results->getResultCount(), 0, 'Clearing the server worked correctly.');
  }
  ...
  protected function regressionTests() {
    ...
    $results = $prepareSearch->execute();
    $this->assertEquals($results->getResultCount(), 4, 'Sorting on field with NULLs returned correct number of results.');
    $this->assertEquals(
      array_keys(
        $results->getResultItems()
      ),
    ...
    $results = $query->execute();
    $this->assertEquals($results->getResultCount(), 3, 'OR filter on field with NULLs returned correct number of results.');
    $this->assertEquals(
      array_keys(
        $results->getResultItems()
      ),
    ...
    $this->assertEquals($results->getResultCount(), 1, 'Negated OR fulltext search returned correct number of results.');
    $this->assertEquals(array_keys($results->getResultItems()), $this->getItemIds(array(3)), 'Negated OR fulltext search returned correct result.');
    $this->assertIgnored($results);
    $this->assertWarnings($results);
    ...
    $results = $this->buildSearch($keys)
                    ->sort('search_api_id', 'ASC')
                    ->execute();
    $this->assertEquals($results->getResultCount(), 2, 'Nested NOT AND fulltext search returned correct number of results.');
    $this->assertEquals(
      array_keys($results->getResultItems()), $this->getItemIds(
      array(
        3,
  ...
  protected function regressionTests2() {
    ...
    $this->assertEquals(array_keys($results->getResultItems()), $this->getItemIds(array(6)), 'Filter on decimal field returned correct result.');
    $this->assertIgnored($results);
    $this->assertWarnings($results);
