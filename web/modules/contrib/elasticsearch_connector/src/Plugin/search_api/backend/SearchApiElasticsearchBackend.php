...
class SearchApiElasticsearchBackend extends BackendPluginBase implements PluginFormInterface, SearchApiElasticsearchBackendInterface {
  ...
  public function viewSettings() {
    ...
    if ($this->server->status()) {
      ...
      if ($ping) {
        $msg = $this->t('The Elasticsearch server was reached successfully');
      }
      else {
    ...
    try {
      ...
      if (!empty($response['errors'])) {
        foreach ($response['items'] as $item) {
          if (!empty($item['index']['status']) && intval($item['index']['status']) > 400) {
  ...
  protected function parseFacets(ResultSet $results, QueryInterface $query) {
    ...
    foreach ($attach as $_key => &$attached_terms) {
      $this->parseDateFacets($index_fields, $_key, $attached_terms);
    }
  ...
  protected function parseDateFacets(array $index_fields, $field_name, array &$terms) {
    ...
    if ($field_type === 'date') {
      foreach ($terms as &$term) {
        ...
        // not milliseconds.
        $term['filter'] = '"' . (str_replace('"', '', $term['filter']) / 1000) . '"';
      }
    }
  }
