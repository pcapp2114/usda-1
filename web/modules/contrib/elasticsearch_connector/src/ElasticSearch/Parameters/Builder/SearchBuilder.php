...
class SearchBuilder {
  ...
  public function build() {
    ...
    $event = $dispatcher->dispatch($buildSearchParamsEvent, BuildSearchParamsEvent::BUILD_QUERY);
  ...
  protected function getSearchQueryOptions() {
    ...
    $event = $dispatcher->dispatch($prepareSearchQueryEvent, PrepareSearchQueryEvent::PREPARE_QUERY);
    ...
    if (!empty($query_options['mlt'])) {
      ...
      if (empty($language_ids)) {
        ...
        $language_ids[] = \Drupal::languageManager()
          ...
          ->getId();
        // For non-translatable entity types, add the "not specified" language
        // to the query so they also appear in the results.
