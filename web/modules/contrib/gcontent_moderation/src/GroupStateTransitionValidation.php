...
class GroupStateTransitionValidation extends StateTransitionValidation implements StateTransitionValidationInterface {
   ...
   *   The group relation enabler plugin manager.
   */
  ...
  public function getValidTransitions(ContentEntityInterface $entity, AccountInterface $user) {
    ...
    // Only act if there are group relationship types for this entity bundle.
    $group_relationship_types = $this->entityTypeManager->getStorage('group_content_type')->loadByPluginId($this->getPluginId($entity));
  ...
  public function isTransitionValid(WorkflowInterface $workflow, StateInterface $original_state, StateInterface $new_state, AccountInterface $user, ?ContentEntityInterface $entity = NULL) {
    ...
    if ($entity) {
      ...
      if ($entity->isNew()) {
        $initial_moderation_state = $workflow->getTypePlugin()->getInitialState($entity);
        $entity->set('moderation_state', $initial_moderation_state->id());
        ...
        $original_revision = $entityStorage->loadRevision($entity->getLoadedRevisionId());
