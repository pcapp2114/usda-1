...
trait CshsOptionsFromHelper {
  ...
  public function settingsForm(array $form, FormStateInterface $form_state): array {
    ...
    $element['allow_unpublished'] = [
      '#type' => 'checkbox',
      '#title' => $this->t('Allow unpublished terms'),
      '#default_value' => $this->getSetting('allow_unpublished') ?? FALSE,
    ];
  ...
  public function formElement(): array {
    ...
    return [
      ...
      '#options' => $this->getOptions((int) ($settings['parent'] ?? 0), $max_depth, $settings['allow_unpublished'] ?? FALSE),
   ...
   * @param bool $allow_unpublished
   *   Whether to allow only published terms, unless configured to allow all.
   *
   * @return \Drupal\cshs\Component\CshsOption[]
   *   Widget options.
   */
  private function getOptions(int $parent = 0, ?int $max_depth = NULL, bool $allow_unpublished = FALSE): array {
    ...
    if (!isset($cache[$cache_id])) {
      ...
      foreach ($this->getVocabularies() as $vocabulary) {
        ...
        foreach ($storage->loadTree($vocabulary->id(), $parent, $max_depth, FALSE) as $term) {
          ...
          // Allow only published terms, unless configured to allow all.
          if ($allow_unpublished || (bool) $term->status) {
