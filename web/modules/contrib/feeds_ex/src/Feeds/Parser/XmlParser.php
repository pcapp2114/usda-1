...
class XmlParser extends ParserBase implements ContainerFactoryPluginInterface {
  ...
  public function mappingFormAlter(array &$form, FormStateInterface $form_state) {
    ...
    $form['mappings']['#header'][] = t('Raw');
    $form['mappings']['#header'][] = t('Inner');

    $mappings = $this->feedType->getMappings();
    foreach ($mappings as $delta => $mapping) {
      foreach ($mapping['map'] as $column => $source) {
        $form['mappings'][$delta]['raw'][$column] = [
          '#title' => t('Raw value'),
          '#type' => 'checkbox',
          '#default_value' => $this->configuration['sources'][$source]['raw'] ?? FALSE,
          '#title_display' => 'invisible',
        ];

        $form['mappings'][$delta]['inner'][$column] = [
          '#title' => t('Inner XML'),
          '#type' => 'checkbox',
          '#default_value' => $this->configuration['sources'][$source]['inner'] ?? FALSE,
          '#title_display' => 'invisible',
        ];
      }
    }
  ...
  public function mappingFormSubmit(array &$form, FormStateInterface $form_state) {
    parent::mappingFormSubmit($form, $form_state);

    $config = $this->getConfiguration();
    $mappings = $form_state->getValue('mappings');

    foreach ($mappings as $key => $mapping) {
      foreach ($mapping['map'] as $subkey => $map) {
        $name = $map['select'];

        $config['sources'][$name]['raw'] = (bool) $mapping['raw'][$subkey];
        $config['sources'][$name]['inner'] = (bool) $mapping['inner'][$subkey];
      }
    }
